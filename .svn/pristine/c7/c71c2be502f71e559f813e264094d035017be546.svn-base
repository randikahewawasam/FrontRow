...
public class UserDB extends FrontRowDBHelper{
	...
	public static final String CONTACT_STYLE="ContactStyle";
	...
	public long addUserDetails(User user){
		...
		try{
			...
			byte contactStyle = (byte) ((user.getContactStyle())?1:0);
			statement.bindLong(12, contactStyle);
	...
	public User getUser(int usrId){
		...
		if(cursor != null){
			if(cursor.moveToNext()){
				...
				byte contactStyle = (byte) cursor.getInt(cursor.getColumnIndex(CONTACT_STYLE));
				if(contactStyle == 1){
					usr.setContactStyle(true);
				}else{
					usr.setContactStyle(false);
				}
	...
	public Boolean getUserContactStyle(String mobileNo){
		Boolean constyle =false;
		SQLiteDatabase db= this.getWritableDatabase();
		Cursor cursor = db.query(TABLE, null, MOBILE_NO+"=?", new String[]{mobileNo}, null, null, null);
		if(cursor != null){
			if(cursor.moveToNext()){

...
public class FrontRowLogin extends BaseActivity implements IFrontResponseReceiver{
	...
	protected void refreshUI() {
		...
		if(response instanceof AuthenticationResponse){			
			...
			if(code == Constants.NETWORK_SUCCESS){
				...
				this.appUsr.setContactStyle(usr.getContactStyle());
				//this.appUsr.setContactStyle(false);
				String versionName= "";
				try {
				...
				if(this.appUsr.isSaved()){
					Enum<AppStatus> status = DataSerializer.isOfflineUser();
					if(status==AppStatus.sucess){
				...
				if(((AuthenticationResponse) response).getResponseCode() == Constants.UNAUTHORIZE){
					txtErrorMsg.setText(getResources().getString(R.string.INVALID_LOGIN));
				}else{
	...
	private void connect(String cmpId,String usrId,String pwd) {
		...
		int i = (b)?0:1;
		AsyncHttpClient httpClient = new AsyncHttpClient(Constants.BASE_URL+"getalldata/"+i);

	...
	protected void refreshUI() {
		...
		if (response instanceof OfflineResponse) {
			...
			if (code == Constants.NETWORK_SUCCESS) {
                ...
                if (offMsg.getType()==Constants.NEW_CLIENT)
                {
				ClientDB clientDb = new ClientDB(this);
				Client client=new Client();
				client.setClientNumber(clientnumber);
				clientDb.updateClient(((OfflineResponse) response).getClient(),ApplicationUser.getSharedInstance().getUserId());
                }
				...
				offMsg.setStatus(Constants.FAILED);
			     if (offMsg.getType()==Constants.NEW_CLIENT)
					...
					ClientDB clientDb = new ClientDB(this);
					//clientDb.open();
					clientDb.deleteClient(client,ApplicationUser.getSharedInstance().getUserId());
	                }

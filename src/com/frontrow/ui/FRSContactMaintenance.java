...
public class FRSContactMaintenance extends BaseActivity implements OnItemSelectedListener, FRSAddNewContactListener {
	...
	protected void onResume() {
		...
		UserDB udb = new UserDB(FRSContactMaintenance.this);
		contactstyle = udb.getUserContactStyle(appUsr.getMobileNnumber());
		...
		SharedMethods.logError("contactType "+Constants.contactType);
		//if(Constants.contactType == ContactType.Initial){
	...
	protected void initializeUI() {
		...
		number2txt = (TextView) findViewById(R.id.number2textid);
  ...
  RelationshipTypesDB reldb=new RelationshipTypesDB(this);
		//contactRelationshipTypeList=AuthDataManager.getSharedInstance().getClientContactList();
		contactRelationshipTypeList=reldb.getRelationshipTypes(Constants.CONTACT_RELATIONSHIP);
		//addressRelationshipTypeList=AuthDataManager.getSharedInstance().getContactAddressList();
		addressRelationshipTypeList=reldb.getRelationshipTypes(Constants.ADDRESS_RELATIONSHIP);
		//phoneRelationshipTypeList = AuthDataManager.getSharedInstance().getContactPhoneList();
		phoneRelationshipTypeList = reldb.getRelationshipTypes(Constants.PHONE_RELATIONSHIP);
	  ...
	  contactstyle = udb.getUserContactStyle(appUsr.getMobileNnumber());
		...
		if(contactstyle){
			...
			number2txt.setText(getResources().getString(R.string.PHONE_NUMBER_2));
		}else{
			addressrelationshipTypeSpinner.setVisibility(View.VISIBLE);
			phonerelationshipTypeSpinner.setVisibility(View.VISIBLE);
			...
			number2txt.setText(getResources().getString(R.string.PHONE_NUMBER));
		}
		addActionListeners();
	}
	...
	/*			getParent().showDialog(Constants.SHOW_DIALOG);
				...
				bundle.putString(Constants.ERROR_D,
						getResources().getString(R.string.DELETE_CONTACT_MSG));
				bundle.putString(Constants.TYPE, Constants.DELETE_CONTACT_TYPE);
		...
		numbersBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				//Constants.contactType = ContactType.Initial;
				if(Constants.contactType == ContactType.Add_Contact){
					if(!contactstyle){
						if(contactId == 0){
							validateButtons();
					...
					Intent intent;
					intent = new Intent(FRSContactMaintenance.this,FRSPhoneList.class);
					intent.putExtras(bundle);
					com.frontrow.ui.TabManager parentActivity = (TabManager) getParent();
					parentActivity.startChild("PhoneList_Activity",
							intent);
				}
			}
		});
	...
	private void validateButtons(){
		...
		b.putString(Constants.ERROR_D, getResources().getString(R.string.ADD_CONTATC_FIRST));
	...
	private NewContact addEditContact(){
		...
		newContact.setIsComposite(contactstyle);
	...
	private void setData() {
		...
		if(contactstyle){
			...
			phoneRelationshipList = reldb.getRelationshipTypes(Constants.PHONE_RELATIONSHIP);
			//addressRelationshipList = AuthDataManager.getSharedInstance().getContactAddressList();
			addressRelationshipList = reldb.getRelationshipTypes(Constants.ADDRESS_RELATIONSHIP);
		}
		//contactRelationshipList =	AuthDataManager.getSharedInstance().getClientContactList();
		contactRelationshipList = reldb.getRelationshipTypes(Constants.CONTACT_RELATIONSHIP);
			...
			for(com.row.mix.beans.Phone tempPhn:phnList){
				if(tempPhn.getPhoneID() == contact.getPrimaryPhone().getPhoneID()){
					for(int i=0;i<phoneRelationshipList.size();i++){
						...
						if(cRelat.getId() == contact.getPrimaryPhone().getPhnRelationshipId()){
			...
			for(Address tempAdd:addressList){
				if(tempAdd.getId() == contact.getPrimaryAddress().getId()){
					for(int i=0;i<addressRelationshipList.size();i++){
						...
						if(cRelat.getId() ==contact.getPrimaryAddress().getAddressRelationshipId()){
